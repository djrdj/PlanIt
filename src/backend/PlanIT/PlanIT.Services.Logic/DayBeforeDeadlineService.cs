...
{
    public class DayBeforeDeadlineService : IInvocable
    {
        ...
        public DayBeforeDeadlineService(IEmailService emailService, ApplicationDbContext dbContext, ILoggger loggger)
        {
        ...
        {
            Logger.LogInformation($"Called: DayBeforeDeadlineService");

            DateTime tomorrow = DateTime.Today.AddDays(1);
            var assignmentsDueTomorrow = _dbContext.Assignments
                .Where(a => a.Deadline.Date == tomorrow.Date)
                .Select(a => new
                {
                    a.AssignmentID,
                    Users = a.UserAssignments.Select(ua => ua.User).ToList()
                })
                .ToList();

            foreach (var assignment in assignmentsDueTomorrow)
            {
                foreach (var user in assignment.Users)
                {
                    string subject = "Deadline Reminder";
                    string body = $@"
                                    <html>
                                    <body>
                                        <div style='font-family: Arial, sans-serif;'>
                                            ...
                                            <p>This is a friendly reminder that you have an assignment due tomorrow. Please make sure to complete it on time.</p>
                                            <p>
                                                <a href='http://softeng.pmf.kg.ac.rs:10152/?assignmentId={assignment.AssignmentID}' style='display: inline-block; background-color: #2E86C1; color: white; padding: 10px 20px; text-align: center; text-decoration: none; border-radius: 5px;'>
                                                    View Assignment
                                                </a>
                                            </p>
                                            <br/>
